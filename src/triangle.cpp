 ...
 *
 * @return The list of Primitive (triangles) that we read from the scene file.
 */
vector<shared_ptr<Primitive>>
create_triangle_mesh_primitive(bool flip_normals, const std::vector<Argument> &ps)
{
    ...
    string filename = "";
    string bkf_on_str = "off";
    string rvo_str = "off";
    string cn_str = "off";
    for (auto &&arg : ps)
    {
        if (arg.getKey() == "filename")
        {
            // Retrieve filename.
            filename = arg.getValue<std::string>();
        }
        if (arg.getKey() == "backface_cull")
        {
            // Retrieve backface ON/OFF
            bkf_on_str = arg.getValue<std::string>();
        }
        if (arg.getKey() == "reverse_vertex_order")
        {
            // Retrieve Reverse vertex order ON/OFF
            rvo_str = arg.getValue<std::string>();
        }
        if (arg.getKey() == "compute_normals")
        {
            // Retrieve compute normals flag
            cn_str = arg.getValue<std::string>();
        }
    }
    ...
    { // Read mesh data from an OBJ file provided in the scene file.
        ...
        //std::cout << ">>> Mesh data loading stub!\n";
        else
    ...
    { // This means we must read the mesh data from the scene file, not from the OBJ file.
        ...
        // std::vector<vec2> uv = {};
        // std::vector<int> vertex_indices = findAttribute(ps, "vertices").getValues<int>();
        // std::vector<int> normal_indices = findAttribute(ps, "normals").getValues<int>();
        // std::vector<int> uvcoord_indices = findAttribute(ps, "uv").getValues<int>();
        // todo iterate through ints and transform each triple in a vec3
        int vSize = vertex_ints.size() / 3;
        ...
        // {
        //     int *currIndices = &indices[i * 3];
        //     for (size_t j = 0; j < 3; j++)
        //     {
        //         int currVertexIndex = currIndices[j];
        //         int *currVertex = &vertex_ints[currVertexIndex * 3];
        //         int *currNormal = &normal_ints[currVertexIndex * 3];
        //         vertices.push_back(vec3(currVertex[0], currVertex[1], currVertex[2]));
        //         normals.push_back(vec3(currNormal[0], currNormal[1], currNormal[2]));
        //         // normals.push_back(vec3(normal_ints[indices[i * 3]], normal_ints[indices[i * 3 + 1]], normal_ints[indices[i * 3 + 2]]));
        //     }

        //     // normals.push_back(vec3(normal_ints[i], normal_ints[i + 1], normal_ints[i + 2]));
        //     // uv.push_back(vec2(uvcoord_ints[i], uvcoord_ints[i + 1]));
        // }
                      ...
                      bool rvo, bool cn, bool fn, /* OUT */ shared_ptr<TriangleMesh> md)
{
    ...
    for (auto idx_tc{0u}; idx_tc < n_texcoords; idx_tc++)
    {
        ...
        md->uvcoords.push_back(vec2{attrib.texcoords[2 * idx_tc + 0],
